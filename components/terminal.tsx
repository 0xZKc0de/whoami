...
export function Terminal({ projects }: TerminalProps) {
    ...
    async function runCommand(input: string, isInitial = false) {
        ...
        try {
            const { output, isClear } = await executeCommand(input, { projects })
            ...
            setIsAnimating(true)
            setVisibleLines(0)
            setCurrentAnimatingEntry(currentIndex)
        } finally {
    ...
    function handleKeyDown(e: KeyboardEvent<HTMLInputElement>) {
        if (e.key === "Enter" && currentInput.trim() && !isAnimating && !isProcessing) {
                ...
                <div className="flex items-center justify-between px-4 py-1.5 bg-zinc-900/80 backdrop-blur-xl border-t border-zinc-800/50 text-[10px] font-mono select-none">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5">
                            <div className={`w-1.5 h-1.5 rounded-full ${isAnimating || isProcessing ? "bg-amber-500/80 animate-pulse" : "bg-emerald-500/80"}`} />
                            <span className="text-zinc-500">
                                {isProcessing ? "Fetching via API..." : isAnimating ? "Processing..." : "Ready"}
                            </span>
                        </div>
